/************************************************************************
  ...
  (c) Copyright Fair Isaac Corporation 2000-2012. All rights reserved.   
      rev. Mar. 2011
************************************************************************/
...
class XPRBexpr
{
 ...
 public:
  ...
  int print() const { return xbprintexpr(NULL,expr,0,0); }
...
class XPRBctr
{
 ...
 public:
  ...
  int setIncludeVars(bool m) { return xbsetincvars(ctrRef, m ? 1 : 0); }
  bool isIncludeVars() { return (xbgetincvars(ctrRef) != 0); }
  int setDelayed(bool d) { return xbsetdelayed(ctrRef, d?1:0); }
  bool isDelayed() { return (xbgetdelayed(ctrRef)!=0); }
  int setIndicator(int dir, const XPRBvar& var)
   { return xbsetindicator(ctrRef, dir, var.getCRef()); }
  int setIndicator(const XPRBvar& var, int dir)
   { return xbsetindicator(ctrRef, dir, var.getCRef()); }
  int setIndicator(int dir)
   { return xbsetindicator(ctrRef, dir, NULL); }
  bool isIndicator() { return (xbgetindicator(ctrRef)!=0); }
  int getIndicator() { return xbgetindicator(ctrRef); }
  XPRBvar getIndVar() { return XPRBvar((struct Xbvar *)xbgetindvar(ctrRef)); }
  ...
  double getCoefficient(const XPRBvar& var)
   { return xbgetcoeff(ctrRef, var.getCRef()); }
  double getCoefficient(const XPRBvar& var1, const XPRBvar& var2)
   { return xbgetqcoeff(ctrRef, var1.getCRef(), var2.getCRef()); }
  ...
  int getSize() { return xbgetctrsize(ctrRef); }
...
private:
  ...
  int print() { return xbprintsol(solRef); }
  int delVar(const XPRBvar& var) { return xbdelsolvar(solRef, var.getCRef()); }
  int setVar(const XPRBvar& var, double val) { return xbsetsolvar(solRef, var.getCRef(), val); }
  int getVar(const XPRBvar& var, double *val) { return xbgetsolvar(solRef, var.getCRef(), val); }
  int getSize() { return xbgetsolsize(solRef); }
  void reset() { if( solRef ) { xbdelsol( solRef ); solRef = NULL; } }
};
...
class XPRBprob
{
 ...
 public:
  ...
  XPRBsol newSol() { return XPRBsol(xbnewsol_prob(prob_ref)); }
  ...
  int lpOptimize(const char *alg="") { return xblpoptimize_prob(prob_ref, alg); }
  int lpOptimise(const char *alg="") { return xblpoptimize_prob(prob_ref, alg); }
  int mipOptimize(const char *alg="") { return xbmipoptimize_prob(prob_ref, alg); }
  int mipOptimise(const char *alg="") { return xbmipoptimize_prob(prob_ref, alg); }
  ...
  int loadMIPSol(double *sol, int ncol, bool ifopt) 
   { return xbloadmipsol_prob(prob_ref, sol, ncol, ifopt?1:0); }
  int loadMIPSol(double *sol, int ncol) 
   { return xbloadmipsol_prob(prob_ref, sol, ncol, 0); }
  int addMIPSol(XPRBsol& sol, const char *name = 0) { return xbaddmipsol_prob(prob_ref, sol.getCRef(), name); }
  int fixGlobals(int ifround = 1) { return xbfixglobals_prob(prob_ref, ifround); }
  int setName(const char *name) { return xbsetprobname_prob(prob_ref, name); }
  ...
  { return xbbegincb_prob(prob_ref, optprob); }
  int endCB() { return xbendcb_prob(prob_ref); }
  ...
  int writeSol(const char *filename = NULL, const char *flags = "") { return xbwritesol_prob(prob_ref, filename, flags); }
  int writePrtSol(const char *filename = NULL, const char *flags = "") { return xbwriteprtsol_prob(prob_ref, filename, flags); }
  int writeBinSol(const char *filename = NULL, const char *flags = "") { return xbwritebinsol_prob(prob_ref, filename, flags); }
  int writeSlxSol(const char *filename = NULL, const char *flags = "") { return xbwriteslxsol_prob(prob_ref, filename, flags); }
  int readBinSol(const char *filename = NULL, const char *flags = "") { return xbreadbinsol_prob(prob_ref, filename, flags); }
  int readSlxSol(const char *filename = NULL, const char *flags = "") { return xbreadslxsol_prob(prob_ref, filename, flags); }
