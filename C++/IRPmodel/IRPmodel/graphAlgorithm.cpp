...
void graphAlgorithm::printGraph(vector<Node*>& graph, IRP &instance, string filename) {
	...
	vector<vector<boost::tuple<int, int, int, int> >> arcs(6);
	...
	string color[] = { "#DDEEF9",  "#AAD5F0","#8DC6EB", "#72B8E7", "#5FADE2","#3498db" };
...
void graphAlgorithm::printGraph(vector<IRP::NodeIRP*>& graph, IRP &instance, string filename) {
	...
	for (IRP::NodeIRP *node : graph) {
		...
		for (IRP::NodeIRP::EdgeIRP* edge : node->getEdges()) {
			if (edge->getValue() != -1) {
				x2 = instance.getMap()->getX(edge->getEndNode()->getId());
				y2 = instance.getMap()->getY(edge->getEndNode()->getId());
				...
				int id = getColor((edge->LoadDel + edge->LoadPick) / instance.getCapacity());
				arcs[id].push_back(boost::make_tuple(
					x1,
					y1,
					x2 - x1,
